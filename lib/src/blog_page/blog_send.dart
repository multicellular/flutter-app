...
class PostBlogDialogState extends State<PostBlogDialog> {
  ...
  List<AssetEntity> _images = List<AssetEntity>();
  File _video;
  ...
  _postBlog() async {
    ...
    if (_video != null) {
      ...
      for (var asset in _images) {
        List<int> imageData = await asset.thumbDataWithSize(300, 300);
...
//            ),
    ...
    return Row(
      children: <Widget>[
        isHideImageBtn
            ? Container()
            : IconButton(
                icon: Icon(Icons.photo), onPressed: () => loadAssets(PickType.all)),
        isHideVideoBtn
            ? Container()
            : IconButton(
  ...
  Future<void> loadAssets(PickType type, {List<AssetPathEntity> pathList}) async {
    ...
    try {
      resultList =
          await PhotoPicker.pickAsset(context: context, maxSelected: 9, rowCount:3, pickType: type, photoPathList: pathList,);
    } catch (e) {
      // error = e.message;
    }
